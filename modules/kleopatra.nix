...
in {
  options.programs.plasma.kleopatra = {
    ...
    "CertificateCreationWizard" = with types;
      mkOption {
        type = submodule {
          options = {
    ...
    "ConfigurationDialog" = with types;
      mkOption {
        type = submodule {
          options = {
  ...
  config =
    mkIf cfg.enable { programs.plasma.files."kleopatrarc" = cfg.kleopatra; };
}
