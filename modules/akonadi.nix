...
in {
  options.programs.plasma.akonadi = {
    ...
    "Compacting" = with types;
      mkOption {
        type = submodule {
          options = {
            "CompactFrequency" = mkOption {
              type = nullOr (either str (enum [ "never" "per_x_messages" ]));
        ...
        type = submodule {
          options = {
            ...
            "textToSpeak" = mkOption {
              type = nullOr str;
              default = ''
                i18nc("%f is a variable for agent. Do not change it", "A message was received from %f")'';
  ...
  config = mkIf cfg.enable { programs.plasma.files."akonadirc" = cfg.akonadi; };
}
